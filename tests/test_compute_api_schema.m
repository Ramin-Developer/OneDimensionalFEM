...
assert(isnumeric(femData.mesh_Size) && isequal(size(femData.mesh_Size), size(numElements)), ...
    'mesh_Size must align with num_Elements shape.');
assert(ischar(femData.q_Type) && ~isempty(femData.q_Type), ...
    'q_Type must be a non-empty normalized char identifier.');
assert(isnumeric(femData.solution_Size) && isequal(size(femData.solution_Size), size(numElements)), ...
    'solution_Size must align with num_Elements shape.');
...
assert(isnumeric(femData.l2_Error_Lin) && isequal(size(femData.l2_Error_Lin), size(numElements)), ...
    'l2_Error_Lin must provide one value per mesh.');
assert(isnumeric(femData.l2_Error_Cub) && isequal(size(femData.l2_Error_Cub), size(numElements)), ...
