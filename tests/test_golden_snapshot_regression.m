...
assert(max(abs(femData.l2_Error_Lin(:) - snapshot.l2ErrorLin(:))) <= tol.errorAbs, ...
    'Linear error snapshot mismatch.');
assert(max(abs(femData.l2_Error_Cub(:) - snapshot.l2ErrorCub(:))) <= tol.errorAbs, ...
    'Cubic error snapshot mismatch.');
assert(max(abs(femData.convergence_Factor_Lin(:) ...
    - snapshot.convergenceFactorLin(:))) <= tol.convAbs, ...
    'Linear reduction-factor snapshot mismatch.');
assert(max(abs(femData.convergence_Factor_Cub(:) ...
    - snapshot.convergenceFactorCub(:))) <= tol.convAbs, ...
    'Cubic reduction-factor snapshot mismatch.');
assert(max(abs(femData.convergence_Order_Lin(:) ...
    - snapshot.convergenceOrderLin(:))) <= tol.convAbs, ...
    'Linear observed-order snapshot mismatch.');
assert(max(abs(femData.convergence_Order_Cub(:) ...
    - snapshot.convergenceOrderCub(:))) <= tol.convAbs, ...
    'Cubic observed-order snapshot mismatch.');
