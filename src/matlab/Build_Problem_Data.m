...
assert((ischar(qType) || (isstring(qType) && isscalar(qType))) && strlength(string(qType)) > 0, ...
    'Build_Problem_Data:InvalidQType', ...
    'qType must be a non-empty char or scalar string.');
assert(~isempty(strtrim(string(qType))), ...
    'Build_Problem_Data:InvalidQType', ...
    'qType must not be empty or whitespace-only.');
assert(~isnumeric(qType), ...
    'Build_Problem_Data:InvalidQType', ...
    'qType must not be numeric.');
...
assert(all(isfinite(loadCoeff(:))), ...
    'Build_Problem_Data:InvalidLoadCoeff', ...
    'loadCoeff must contain only finite values.');
assert(all(isfinite(qCoeff(:))), ...
    'Build_Problem_Data:InvalidQCoeff', ...
    'qCoeff must contain only finite values.');
assert(all(isreal(loadCoeff(:))), ...
    'Build_Problem_Data:InvalidLoadCoeff', ...
    'loadCoeff must contain only real values.');
assert(all(isreal(qCoeff(:))), ...
    'Build_Problem_Data:InvalidQCoeff', ...
    'qCoeff must contain only real values.');
assert(numel(loadCoeff) >= 1, ...
    'Build_Problem_Data:InvalidLoadCoeff', ...
    'loadCoeff must contain at least one coefficient.');
assert(numel(qCoeff) >= 1, ...
    'Build_Problem_Data:InvalidQCoeff', ...
    'qCoeff must contain at least one coefficient.');
...
assert(isreal(delta) && isreal(P), ...
    'Build_Problem_Data:InvalidBoundaryData', ...
    'delta and P must be real scalar values.');
