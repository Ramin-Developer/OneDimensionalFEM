...
\begin{enumerate}
	...
	\end{itemize}
\end{enumerate}
Define the following measures of solution error:
...
\begin{enumerate}
	...
	\item The linear and cubic solutions converge as $\mathcal{O}(h^2)$ and $\mathcal{O}(h^4)$, respectively, consistent with
	\cref{eqn:FEM_Error_To_h_And_Exact_Sol}.

	\item The cubic errors reach the roundoff floor when the analytical solution is itself a cubic polynomial.
...
\end{enumerate}	\text{}
%
\Cref{lst:Estimate_Error} computes each finite element solution's $L^2$ error norm.
