...
\begin{lemma}	\label{lem:Cauchy-Schwarz}
	The $L^2$ inner product satisfies the Cauchy--Schwarz inequality:
...
\begin{definition}	\label{def:H1_Norm}
	Let $\left\| f \right\|_{ H^1 }$ denote \gls{H1Normf}. Then
	\begin{equation}	\label{eqn:H1Norm}
		\glssymbol{H1Normf} = \sqrt{ \glssymbol{L2Normf}^2 + \big\| f^\prime \big\|_{ L^2 }^2 }.
	\end{equation}
\end{definition}
While $\|f\|_{L^2}$ measures only the magnitude of $f$, $\|f\|_{H^1}$ measures both $f$ and its first derivative. The $L^2$
norm of the first derivative defines the $H^1$ seminorm:
...
\end{definition}
\begin{remark}
	A norm is zero if and only if its argument is the zero function. The quantities $\vert f \vert_{ H^1 }$ and
	$\vert f \vert_{ H^2 }$ can vanish for nonzero functions and are therefore seminorms.\footnote{For example, for a linear
	polynomial $f = ax + b$, we
...
\end{definition}
\begin{remark}
	On $H_0^1$, \glssymbol{aForm} is an inner product: it is the $L^2$ inner product of the derivatives of its arguments.
