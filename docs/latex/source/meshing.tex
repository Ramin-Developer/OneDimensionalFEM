...
\begin{definition}
	\Gls{meshing}: is a process where we divide the whole region
	into smaller pieces, elements in \gls{FEM}-terminology. By the end of this process we have then chosen No. of elements, \gls{N} and
	located the $N + 1$ \glspl{nodePoint}. Another term for \gls{meshing} is \gls{discretization}.
\end{definition}
\begin{remark}
	Two \gls{adjacentElements} $\Omega_e$ and $\Omega_{e + 1}$ have then one and only one common point, $x_{e + 1}$.
\end{remark}
\begin{definition}
	\Gls{uniformMeshing}: A \gls{meshing} is uniform if all \glspl{element} are of the same size. In the one-dimensional case in
	hand the node points are then equidistant
\end{definition}
\begin{remark}
	It is common practice to pre-mesh the \gls{domain}, solve the problem, then analyze the solution, return to the initial
	\gls{meshing} and make it finer/grosser in the areas with rapid/slow solution change. This process, \gls{adaptivemeshing},
	improves the solution and may be carried out iteratively. It will also result in a \gls{nonUniformMeshing}.
\end{remark}
\begin{figure}
	...
	\label{fig:Meshing}
\end{figure}
Consider now an approximation to the or \gls{exactSol} given by
