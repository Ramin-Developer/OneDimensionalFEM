...
The stiffness matrix in \cref{eqn:Stiffness_Matrix_FEM_Linear} may also be derived by a direct or a formal approach.
...
We can also derive the element stiffness matrix in \cref{eqn:Stiffness_Matrix_FEM_Linear} formally. This approach applies to a
...
\end{equation}
Applying \cref{eqn:Stress_Formal,eqn:Strain_Formal}, the strain energy is
...
%
\begin{remark}
	The element stiffness matrix in \cref{eqn:Stiffness_Matrix_Formal_1} generalizes to other element types. It can also be derived
	from the principle of \gls{minPotentialEnergy} or the \gls{GalerkinMethod}.
\end{remark}
...
\end{equation}
%
which agrees with the direct result in \cref{eqn:Stiffness_Matrix_Direct}. Finally, combining
