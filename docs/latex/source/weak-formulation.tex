...
\begin{equation} \label{eqn:Weak_Formulation}
	\int_{x = 0}^{1} \gls{q} \glsentryname{uPrime} \frac{ d \glssymbol{wi} }{dx} \dd x = P  \gls{wi} \Big \vert_{x = 1} + \int_{x = 0}^{1} \gls{f} \gls{wi} \dd x.
\end{equation}
%
...
\begin{problem}	\label{prb:Normalized_Weak_Problem}
	Find $u \in \{v \in H^1(0,1):v(0)=\delta\}$ such that
	\begin{equation}	\label{eqn:Normalized_Weak_Diff_Eq}
		\int_0^1 q u' w' \dd x = \int_0^1 f w \dd x + P w(1)
	\end{equation}
	for every $w \in H^1(0,1)$ satisfying $w(0)=0$.
\end{problem}
