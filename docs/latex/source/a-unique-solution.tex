...
unique solution if and only if determinant of the coefficient matrix is non-zero.
...
\begin{remark}
	...
	$\gls{matA}$ is \gls{singular}, it can not be \gls{posDef}. These statements may be summarized as
	\begin{subequations}
		\begin{align} 
...
For proving uniqueness of \gls{FEM} it is then enough to show that the \gls{stiffnessMatrix} is positive-definite.\\
First note that according to \cref{eqn:Stiffness_Matrix}
\begin{equation}
	\gls{K} = \gls{K}^T \Longleftrightarrow \glsentryname{Kij} = K_{j,i}.
\end{equation}
Furthermore
\begin{equation}
	\begin{aligned}
		\gls{v}^{T} \gls{K} \gls{v}  &= \sum \limits_{i = 1}^{\gls{M}} \sum \limits_{j = 1}^{\gls{M}}
			\gls{vi} \glsentryname{Kij} v_j \\
		&= \sum \limits_{i = 1}^{\gls{M}} \sum \limits_{j = 1}^{\gls{M}} \gls{vi} \Biggl[ \int_{x = 0}^{1}
			\gls{q} \glsentryname{phiiPrime} \glsentryname{phijPrime} \dd x \Biggr] v_j \\
		&= \sum \limits_{i = 1}^{\gls{M}} \Biggl[ \sum \limits_{j = 1}^{\gls{M}} \int_{x = 0}^{1}
		\gls{q} \glsentryname{phiiPrime} \glsentryname{phijPrime} \dd x \Biggr] \gls{vi} v_j.
	\end{aligned}
\end{equation}
Now exchange the order of integration and summations to get
\begin{equation} \label{eqn:Pos_Def_Simplified}
	\begin{aligned}
		\gls{v}^{T} \gls{K} \gls{v}  &= \int_{x = 0}^{1} \gls{q} \Biggl[ \sum \limits_{i = 1}^{\gls{M}} \gls{vi} \glsentryname{phiiPrime} \Biggr]
		\Biggl[ \sum \limits_{j = 1}^{\gls{M}} v_j \glsentryname{phijPrime} \Biggr] \dd x \\
		&= \int_{x = 0}^{1} \gls{q} \Biggl[ \sum \limits_{i = 1}^{\gls{M}} \gls{vi} \glsentryname{phiiPrime} \Biggr]^{2} \dd x > 
		0, \quad \forall \gls{v} \neq 0.
	\end{aligned}
\end{equation}
\Gls{stiffnessMatrix} satisfies both conditions in \cref{eqn:Pos_Def_Prop_1,eqn:Pos_Def_Prop_2}, and is therefore positive-definite.
%
