...
\end{equation}
%
\subsection[Stability of the finite element solution]{Stability of the \glssymbol{FEM}-Solution}	\label{cha:Stability_FEM}
...
\end{equation}
This tells us that the error in finite element solution is orthogonal to the space of functions \glssymbol{spaceWh}, when measured in the
$a$-Inner Product.
%
...
If we measure \textsl{energy} of this error we see that
	...
	\end{align}
Transition from \cref{eqn:Energy_a_Form_1} to \cref{eqn:Energy_a_Form_2} follows as a consequence of the bilinearity and
...
\begin{remark}
	...
	of the error associated with the finite element solution has the least \glsentryname{L2Norm}, i.e., the least \emph{energy}. This is the so-called
	\emph{best approximation property}.
\end{remark}

\subsection{An Estimate for FEA Error Stability}
The error estimate in (\ref{eqn:Energy_a_Form_4}) gives the magnitude of the derivative of the finite element error relative to other
...
\end{equation}
\begin{remark}
	\Cref{eqn:Energy_a_Form_4} gives an error estimate for the finite element solution relative to the error associated with any other member
...
We are finally in a position to compute an error estimate for the finite element solution which shows how the error depends upon the
...
\begin{problem}	\label{prb:Dual_Problem}
	...
	\begin{subequations}
		\begin{align}
			...
			\intertext{where \glssymbol{FEAerror} is the finite element error defined by}
...
\begin{enumerate}
	...
	The finite element error is an element of the space of test functions for the dual problem, $e \in \glssymbol{spaceW}$.
	
	\item \label{itm:Dual_Norm_Relation}
	The norms of the dual field $\phi$ and the finite element error $e$ are related by\footnote{This property is one of the main reasons for
	...
	There exists an element $\glsname{dualWeakInt} \in \glssymbol{spaceSh}$ (and consequently also in $\glssymbol{spaceWh}$) such that the interpolation
...
\end{equation}
By orthogonality of the finite element error to the space of functions $\mathcal{W}^h$ we then have
...
\end{equation}
\Cref{eqn:FEM_Error_in_H1} estimates the interpolation error magnitude associated with the dual problem. Applying this and
...
\end{equation}
We may now apply the error stability estimate in (\ref{eqn:H1_Error_Estimate_FEM}) together with the interpolation error estimate
