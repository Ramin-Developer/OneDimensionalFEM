...
To idealize the problem we assume the following assumptions:
\begin{enumerate}
	\item The displacement is small.
	\item The material is homogeneous and isotropic with modulus of elasticity $E$.
	\item The load is inside the linear elastic range, the material follows Hook's law.
	\item The bar is in a state of uni-axial stress. This idealization is not exact since the state of stress at the fixed end of
	the bar is in three dimensions due to the Poisson's effects.\footnote{This is a ratio named after Sim\'{e}on Poisson.
...
\end{enumerate}	\text{}
Consider now an infinitesimal material element of length $dz$. Using equilibrium of forces in the element's longitudinal axis
we get
...
\begin{subequations}
	...
	\end{align}
\end{subequations}
Combining \cref{eqn:Stress_Strain_1,eqn:Strian_Displacement} we have the stress-strian equation
...
\end{equation}
Since the left end of the bar is walled, the displacement here is zero. However, this text is meant to serve as a
general approach to FEM, so it makes sense to define a constant as the value of displacement at the left boundary. Thus
\begin{equation}
...
\begin{remark}
	...
	or \gls{naturalCondition}. In a \gls{1DimProblem}, it specifies the solution's first derivative. Thus
	\cref{eqn:Non_Normalized_Normal_Cond} gives the first derivative's value on the \gls{rightBoundary}.
\end{remark}
%
